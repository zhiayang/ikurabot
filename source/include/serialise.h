...
namespace ikura::serialise
{
	...
	constexpr uint8_t TAG_HASHMAP       = 0x0A;
	...
	struct Writer
	{
		...
		{
			// how to ensure? no point. just ensure for the tag + size first.
			ensure(9); tag(TAG_HASHMAP);
	...
	struct Reader
	{
		...
		std::optional<T> read()
		{
			...
			else if constexpr (is_unordered_map<T>::value)  { if(tag() != TAG_HASHMAP) return { }; }
